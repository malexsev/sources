namespace Cure.Notification
{
    ...
    public class Before30DaysToUserEmailNotification : BaseNotification
    {
        ...
        private const string bodyTemplate =
            @"От Вас не поступало информации  о купленных билетах. Вы записаны на лечение с {0}.<br />" +
            @"Визы оформлены?<br />" +
            @"Когда планируете покупать билеты?<br />" +
            @"Просим Вас так же иметь ввиду, что если не будет ответа на данное письмо, тогда Администрация клиники имеет право перенести Ваше лечение на другую свободную дату."; // 0 - DateFrom
