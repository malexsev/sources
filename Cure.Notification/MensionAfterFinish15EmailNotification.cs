namespace Cure.Notification
{
    ...
    public class MensionAfterFinish15EmailNotification : BaseNotification
    {
        private string subject;
        private string body;
        private IEnumerable<Order> orders;

        public MensionAfterFinish15EmailNotification(HttpServerUtilityBase server)
            : base(server)
        {
            try
            {
                ...
                this.subject = dal.GetSettingByCode("EmailTemplateMensionRequiredAfterFinish15Subject").Value;
                this.body = dal.GetSettingByCode("EmailTemplateMensionRequiredAfterFinish15Body").Value;

                DateTime day = DateTime.Today.AddDays(-15);
        ...
        private void Log(string result, string recipient)
        {
            var notify = new NotificationLog()
            {
                ...
                Description = "Оповещение требуется отзыв через 15 дней после лечения",
                Contacts = recipient,
